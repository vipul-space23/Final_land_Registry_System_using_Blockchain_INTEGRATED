...
// //     });
// // });
// // In propertyController.js - Update confirmSale to record history
// export const confirmSale = asyncHandler(async (req, res) => {
//     const { buyerWalletAddress, transactionHash } = req.body;
...
//     // ✅ Record this sale in history BEFORE updating owner
//     const saleRecord = {
//         from: property.owner, // Previous owner ID
//         fromWallet: property.ownerWalletAddress, // Previous owner wallet
//         to: req.user._id, // New owner ID
//         toWallet: buyerWalletAddress, // New owner wallet
//         soldPrice: property.price,
//         transactionHash: transactionHash,
//         soldAt: new Date()
//     };

//     // Add to sale history array
//     if (!property.saleHistory) {
//         property.saleHistory = [];
//     }
//     property.saleHistory.push(saleRecord);

//     // Update current ownership
//     property.previousOwner = property.owner;
//     property.owner = req.user._id;
//     property.ownerWalletAddress = buyerWalletAddress;
//     property.ownerName = req.user.name;
...
//         historyCount: updatedProperty.saleHistory.length
//     });
...
// /**
//  * @desc    Update property with map data (lat, lng, geometry)
//  * @route   PUT /api/properties/:id/map-data
//  * @access  Private (Owner)
//  */
// export const updatePropertyMapData = asyncHandler(async (req, res) => {
//   const { latitude, longitude, geometry } = req.body;

//   if (latitude == null || longitude == null || !geometry) {
//     res.status(400);
//     throw new Error('Latitude, longitude, and geometry are required.');
//   }
...
//   property.geometry = geometry; // Save the GeoJSON object
...
//     }
// });
// // In propertyController.js - Add new function to get sold properties
// export const getMySoldProperties = asyncHandler(async (req, res) => {
//     console.log('🔍 Getting sold properties for user:', req.user._id);
    
//     // Find properties where the user was the previous owner (they sold it)
//     const soldProperties = await Property.find({ 
//         previousOwner: req.user._id,
//         status: 'sold'
//     })
//     .populate('owner', 'name email walletAddress phone') // Current owner (buyer)
//     .populate('previousOwner', 'name email walletAddress') // Previous owner (seller - current user)
//     .select('-documentHashes')
//     .sort({ soldAt: -1 }); // Most recent first
...
export const confirmSale = asyncHandler(async (req, res) => {
    ...
    const saleRecord = {
        from: property.owner,
        fromWallet: property.ownerWalletAddress,
        to: req.user._id,
        toWallet: buyerWalletAddress,
        soldPrice: property.price,
        transactionHash: transactionHash,
        soldAt: new Date()
    };
...
/**
 * @desc    Get properties sold by the current user
 * @route   GET /api/properties/my-sold
